...
var util=new Utility();function Ship(x,y){this.position={x:x,y:y}
...
this.draw()}}
var Game=function(){this.over=!1;this.bullets=[];this.enemys=[];this.stars=[];this.ship=undefined;this.point=0;this.start=function(){if(!this.over){this.reset();info()
...
this.addPoint=function(size){(size<=25?this.point+=2:this.point+=5);document.getElementById('point').innerHTML=this.point}}
var setup=util.setup();var width=setup.width;var height=setup.height;var canvas=setup.canvas;var c=setup.context;var total_frames=0;document.querySelector("canvas").addEventListener('touchmove',function(event){try{game.ship.move(event,!0)}
